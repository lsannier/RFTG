...
public interface InventoryDisponibleRepository extends CrudRepository<Inventory, Integer> {
    ...
    @Query(value = "SELECT MIN(inventory_id) FROM inventory WHERE film_id = :filmId AND inventory.existe = true AND NOT EXISTS (SELECT rental.inventory_id FROM rental WHERE rental.inventory_id = inventory.inventory_id AND rental.return_date IS NULL);", nativeQuery = true)
